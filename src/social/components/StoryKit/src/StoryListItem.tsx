...
export const StoryListItem = ({
  ...
  useEffect(() => {
    ...
    let data = [...content];
    data.map((storyItem, storyIndex) => {
      if (isPrevious) {
        storyItem.finish = 1;
        if (storyIndex === content.length - 1) {
          storyItem.finish = 0;
        }
      } else {
        storyItem.finish = 0;
      ...
      <View style={[styles.container]}>
        ...
        <View style={styles.flexCol}>
          <View style={styles.animationBarContainer}>
            {content.map((_item, barIndex) => {
              return (
                <View key={barIndex} style={[styles.animationBackground]}>
                  <Animated.View
                    style={[
