...
| `sync-spacing.mjs`        | idempotent | Vendor cleverden `/layout/spacing` (px) → `src/core/design/tokens/spacing.ts` (names normalized to web style: `s2`, `m1`…). Re-run on design drops.                                                                                                   |
...
| `extract-geometry.mjs`    | idempotent | Extract web chat CSS geometry per selector (resolve `var(--asc-spacing-*)` + rem→px) → `scripts/port/geometry.json`. A _guide_ for the styles.ts port.                                                                                                |
| `sync-geometry.mjs`       | idempotent | Vendor SoT Figma geometry (`uikit.tokens.json`, per-component sizes/padding/radius/gaps) → `src/core/design/tokens/geometry.json`. The geometry _authority_.                                                                                          |
| `check-geometry.mjs`      | verifier   | **Gate #5** — assert each mapped RN component's styles match the SoT geometry (values resolved from `geometry.json` via `geometry-map.json`). Unported components skip.                                                                               |
...
| `check-web-parity.mjs`    | verifier   | Per-milestone completeness vs the **local web** tree. Reports covered/missing/skip; fails on UNACCOUNTED, DRIFT, MISSING. `--milestone=N` gates a phase; `--no-stubs` also fails on unfinished `PORT STUB`/`TODO(port)` markers.                      |
