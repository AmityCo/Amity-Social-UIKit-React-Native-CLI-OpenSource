...
| `extract-geometry.mjs`    | idempotent | Extract web chat CSS geometry (size/spacing/radii/typography) per selector, resolving `var(--asc-spacing-*)` + rem→px, → `scripts/port/geometry.json`. Guides the styles.ts port.                                                                     |
